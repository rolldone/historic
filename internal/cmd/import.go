...
func newImportCommand() *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			response := importOutput{Command: "import", OK: true, Data: importData{ID: topic.ID.String(), Title: topic.Title, Storage: "open", Path: workspace.RelativePath(topic.Path)}, Error: nil}
			if jsonOutput {
