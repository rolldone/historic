...
func TestSyncMetaReconcilesRenameDeleteMoveAndClassification(t *testing.T) {
	...
	updated, err := os.ReadFile(filepath.Join(topic, "_meta.md"))
	if err != nil {
		t.Fatal(err)
	}
	text := string(updated)
	if strings.Contains(text, "stale.md") || strings.Contains(text, "old.md") {
		t.Fatalf("stale links remain: %s", text)
	}
	if !strings.Contains(text, "- [renamed.md](./renamed.md)") || !strings.Contains(text, "- [moved.md](./docs/moved.md)") || !strings.Contains(text, "- [manual.txt](./manual.txt)") {
		t.Fatalf("current links missing: %s", text)
	}
	if !strings.Contains(text, "keep this") {
		t.Fatalf("non-generated section changed: %s", text)
	}
...
func TestSyncMetaReconcilesManagedToAsset(t *testing.T) {
	...
	updated, _ := os.ReadFile(filepath.Join(topic, "_meta.md"))
	text := string(updated)
	filesSection := text[strings.Index(text, "## Files"):strings.Index(text, "## Assets")]
	assetsSection := text[strings.Index(text, "## Assets"):]
	if strings.Contains(filesSection, "changed.md") || !strings.Contains(assetsSection, "changed.md") {
		t.Fatalf("classification not reconciled: %s", text)
	}
}
